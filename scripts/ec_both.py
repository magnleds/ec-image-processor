    ...
    for i, src in enumerate(files, 1):
        ...
        try:
            ...
            if ext == 'png':
                ...
                canvas.save(out_path, 'JPEG', quality=args.jpg_quality, optimize=True)

            out_kb = os.path.getsize(out_path) // 1024
            saved  = orig_kb - out_kb
            saved_total += saved
            ratio  = int((1 - out_kb / max(orig_kb, 1)) * 100)
            print(f'[{i}/{total}] {rel}  {orig_kb}KB → {out_kb}KB  (-{ratio}%)', flush=True)
        except Exception as e:
            print(f'[{i}/{total}] {rel} 失败: {e}', flush=True)

    print(f'完成！共处理 {total} 张，节省 {saved_total}KB', flush=True)
